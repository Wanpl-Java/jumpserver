...
class MessageFeedback(JMSBaseModel):
    ...
    user = models.ForeignKey(
        'users.User', on_delete=models.CASCADE, related_name='chat_ai_feedbacks',
        verbose_name=_('User')
    )
    org_id = models.CharField(max_length=36, db_index=True, verbose_name=_('Organization'))
    rating = models.CharField(max_length=16, choices=Rating.choices, verbose_name=_('Rating'))
    reason = models.CharField(max_length=64, blank=True, default='', verbose_name=_('Reason'))
    comment = models.CharField(max_length=1000, blank=True, default='', verbose_name=_('Comment'))

    class Meta:
        db_table = 'chat_ai_message_feedback'
        ordering = ('-date_updated',)
        indexes = [models.Index(fields=('user', 'org_id', '-date_updated'))]
        verbose_name = _('Chat AI message feedback')
