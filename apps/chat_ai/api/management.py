...
from chat_ai.models import AgentRun, ApiCallAudit, Message, MessageFeedback, ScheduledReport
...
class ChatAIStatsView(APIView):
    ...
    def get(self, request):
        ...
        feedback = MessageFeedback.objects.filter(
            org_id=org_id, date_updated__gte=since,
        ).aggregate(
            total=Count('id'),
            positive=Count(
                'id', filter=Q(rating=MessageFeedback.Rating.POSITIVE)
            ),
            negative=Count(
                'id', filter=Q(rating=MessageFeedback.Rating.NEGATIVE)
            ),
        )
        top_operations = list(
