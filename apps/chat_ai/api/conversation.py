...
from chat_ai.models import (
    AgentRun, Approval, Conversation, Message, MessageFeedback, MessageFile,
    MessageImage,
)
...
from .serializers import (
    ...
    MessageFeedbackSerializer, MessageSerializer, RegenerateMessageSerializer,
...
class ConversationViewSet(JMSModelViewSet):
    ...
    def messages(self, request, pk=None):
        conversation = self.get_object()
        queryset = conversation.messages.select_related('feedback').prefetch_related(
            'images', 'files'
        ).all()
    ...
    @extend_schema(request=MessageFeedbackSerializer, responses=MessageFeedbackSerializer)
    @action(
    ...
    def message_feedback(self, request, pk=None, message_id=None):
        ...
        if request.method == 'DELETE':
            MessageFeedback.objects.filter(
                message=message, user=request.user, org_id=str(current_org.id)
            ).delete()
