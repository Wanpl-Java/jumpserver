...
所有接口必须登录，并携带可信组织上下文。用户只能访问当前组织中属于自己的 Conversation 和 Approval。
...
| POST/DELETE | `/api/v1/chat-ai/conversations/{id}/messages/{message_id}/feedback/` | 提交或删除点赞、点踩、原因和备注 |
...
| GET | `/api/v1/chat-ai/stats/?days=30` | 超级管理员查看当前组织用量、Token、反馈、热门操作和定时任务统计 |
...
- `apps/chat_ai/models.py`：Conversation、Message、AgentRun、ApiCallAudit、Approval、Feedback 和 ScheduledReport。
- `apps/chat_ai/api/`：REST、SSE、取消、重新生成、反馈、后台任务、定时报告、统计、审批和 Registry 刷新。
